...
def build_index(
    dataset_root: str, index_path: Optional[str] = None, overwrite: bool = False
) -> None:
    """
    ...
    # Eksplicitno postavi mtime na sada
    try:
...
def get_record(index_path: Optional[str], record_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        c = conn.cursor()
        c.execute(
            "SELECT record_id, category, path, n_frames, maps_path FROM records WHERE record_id = ?",
            (record_id,),
        )
        row = c.fetchone()
        if row:
            return dict(
                zip(["record_id", "category", "path", "n_frames", "maps_path"], row)
            )
        return None
    finally:
        conn.close()
...
def list_records(
    index_path: Optional[str] = None, category: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if category:
            c.execute(
                "SELECT record_id, category, path, n_frames, maps_path FROM records WHERE category = ?",
                (category,),
            )
        else:
            c.execute(
                "SELECT record_id, category, path, n_frames, maps_path FROM records"
            )
        rows = c.fetchall()
        return [
            dict(zip(["record_id", "category", "path", "n_frames", "maps_path"], row))
            for row in rows
        ]
    finally:
        conn.close()
