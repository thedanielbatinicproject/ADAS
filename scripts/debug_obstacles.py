#!/usr/bin/env python3
"""Debug obstacle detection on a single DADA-2000 video.

Shows detected and tracked obstacles overlaid on the video stream.
No collision risk or lane detection is run (unless --with-lanes is passed).

Usage
-----
python scripts/debug_obstacles.py --category-id 1 --video-id 1
python scripts/debug_obstacles.py --category-id 1 --video-id 1 --with-lanes
python scripts/debug_obstacles.py --category-id 1 --video-id 1 --max-frames 300
"""
...
from adas.obstacle_detection import Detector, SimpleTracker  # noqa: E402
from adas.ui.overlays import draw_obstacles  # noqa: E402


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Obstacle detection debug viewer.")
    ...
    p.add_argument(
        "--with-lanes", action="store_true",
        help="Also run lane detection and show overlay.",
    )
    p.add_argument(
...
def main() -> None:
    ...
    lane_func = None
    if args.with_lanes:
        from adas.lane_detection import process_frame, draw_lanes
